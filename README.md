...
│   └── tui.sh              # TUI 引擎：纯 ANSI/XTerm 卡片式交互（无外部依赖）
...
默认运行 `./vps_setup.sh` 时，进入**主菜单**，提供 7 个编号操作与退出。

### 终端 UI（TUI）模式

当 stdin/stdout 均为 TTY、`TERM` 非 `dumb`、终端宽度 ≥ 60 列、且未使用 `-n`/`-a` 时，脚本会自动启用 **Smart TV / BIOS 风格的卡片式 TUI**，进入备用屏幕并渲染带高亮焦点的边框菜单：

```
╔════════════════════════════════════════════════════════════════════════════╗
║ ❖  VPS 一键装机 BIOS 控制台 v2.0.0                                debian | x86_64 ║
╠════════════════════════════════════════════════════════════════════════════╣
║ 📌 请选择操作模式 (方向键/鼠标点击选择):                                                    ║
║                                                                            ║
║   ▶  1 . 🚀 完整向导配置 (Guided Setup Wizard) [推荐首次装机]                 ║
║     2 . 🛠️  模块化分项配置 (Configure by Section) [按模块微调]                         ║
║     3 . 📋 预览当前配置 (Review Configuration) [查看所有生效项]                          ║
║     4 . 🔄 加载/重置配置 (Manage Config File) [重置或重新读取]                           ║
║     5 . ⚡ 开始执行安装 (Start Installation) [确认并立即执行]                            ║
║     6 . 📊 查看模块状态 (Check Module Status) [查询已完成列表]                           ║
║     7 . 🚪 退出装机向导 (Exit Setup Wizard) [退出程序]                                ║
║                                                                            ║
╠════════════════════════════════════════════════════════════════════════════╣
║  [↑/↓] 选择  [Enter] 确认  [b/Esc] 返回  [🖱 点击] 直达                               ║
╚════════════════════════════════════════════════════════════════════════════╝
```
...
|------|------|
| `↑` / `↓` | 在选项间移动焦点（卡片高亮跟随） |
| `←` / `→` / `Tab` / `Space` | Yes/No 控件左右切换 |
| `Enter` | 确认当前焦点项 |
| `Esc` / `b` | 返回上一层（文本输入框中 `b` 为普通字符，返回请用 `Esc`） |
| `1`-`9` | 直接跳到对应编号选项 |
| 鼠标点击 | 支持 SGR 1006 协议的终端可点击卡片直达 |
| 滚轮 | 部分终端支持滚轮上下移动焦点 |

退出时脚本会自动恢复原屏幕、光标和鼠标模式。终端宽度 < 60 列、非 TTY、CI、`-n`、`-a` 等场景会自动降级为下方传统文本菜单。

### 传统文本菜单模式

窄终端或非 TUI 环境下，主菜单以编号文本形式呈现：
...
- `数字编号` — 菜单类提示中直接输入编号选择
- 在 TUI 模式下还支持 `↑/↓` 移动焦点、`Esc` 返回、鼠标点击直达（见上文 TUI 操作键表）
...
```
╔════════════════════════════════════════════════════════════════════════════╗
║ ❖  模块化分项配置 — BIOS 控制台                                      debian | x86_64 ║
╠════════════════════════════════════════════════════════════════════════════╣
║ 📌 请选择需要单独配置或调整的项目:                                                         ║
║                                                                            ║
║   ▶  1 . 1) 🌐 系统基础配置 (Step 1: Hostname / Timezone / DNS)         ║
║     2 . 2) 👤 用户账户配置 (Step 2: Username / SSH Key / Password)                ║
║     3 . 3) 🧹 清理与优化配置 (Step 3: Swap / Snap / Cache / Journal)               ║
║     4 . 4) 🔒 SSH 加固配置 (Step 4: SSH Port / Root Login)                      ║
║     5 . 5) 🛡️  安全组件配置 (Step 5: Fail2ban / Auditd / MAC)                    ║
║     6 . 6) 🐳 可选服务配置 (Step 6: Docker / Node.js & NPM)                       ║
║     7 . 7) 💾 备份与监控配置 (Step 7: Backup / Node Exporter)                      ║
║     8 . 0) 🔙 返回主菜单                                                         ║
║                                                                            ║
╠════════════════════════════════════════════════════════════════════════════╣
║  [↑/↓] 选择  [Enter] 确认  [b/Esc] 返回  [🖱 点击] 直达                               ║
...
选择 5 跳出主菜单，按顺序执行所有已启用模块（或 `--modules` 指定的子集）。执行前会再次弹出 **最终确认 Review**：

- TUI 模式下渲染左右双卡片 `是 / YES` 与 `否 / NO`，可用 `←/→` 切换、`Enter` 确认。
- 传统模式下输入 `Y` 继续，`n` 取消回主菜单。
...
本项目使用 **纯 ANSI/XTerm 控制序列** 实现卡片式 TUI，**不依赖任何外部 UI 库**（无需 `whiptail`、`newt`、`dialog` 等）。所需能力均由标准终端提供：

- 备用屏幕缓冲区（`\e[?1049h` / `\e[?1049l`）
- 光标显示控制（`\e[?25l` / `\e[?25h`）
- SGR 1006 鼠标事件追踪（`\e[?1000h\e[?1006h`）
- 方向键、Enter、Esc、Tab、Space、数字快捷键解析
- SGR 颜色与卡片高亮渲染

退出 TUI 时脚本会通过 `trap ... EXIT INT TERM` 保证在任何情况下恢复原屏幕、光标和鼠标模式，避免终端残留异常状态。
